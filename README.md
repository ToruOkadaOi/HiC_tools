...
- <a name="calder">[Calder](https://github.com/CSOgroup/CALDER)</a> - multi-scale A/B compartment detection. A complete hierarchy of compartment subdomains. Computes whole-chromosome contact similarities, cluster domains using a divisive hierarchical clustering, reorder dendrograms, estimate the likelihood of nested subdomains. Nested domain boundaries are likely associated with loops or TADs. Outperforms adapted k-means, robust to noise. Applied to over 100 cell lines, characterization of domain repositioning. <details>
  <summary>Paper</summary>
  Liu, Yuanlong, Luca Nanni, Stephanie Sungalee, Marie Zufferey, Daniele Tavernari, Marco Mina, Stefano Ceri, Elisa Oricchio, and Giovanni Ciriello. “Systematic Inference and Comparison of Multi-Scale Chromatin Sub-Compartments Connects Spatial Organization to Cell Phenotypes.” Nature Communications 12, no. 1 (May 10, 2021): 2439. https://doi.org/10.1038/s41467-021-22666-3.
</details>
...
- <a name="gothic">[GOTHiC]([https://www.bioconductor.org/packages/release/bioc/html/GOTHiC.html](https://github.com/borimifsud/GOTHiC))</a> - R package for peak calling in individual HiC datasets, while accounting for noise. [Bioconductor package](https://www.bioconductor.org/packages/release/bioc/html/GOTHiC.html) <details>
    <summary>Paper</summary>
...
- <a name="deeptad">[deepTAD](https://github.com/xiaoyan-wang99/deepTAD)</a> - a CNN and transformer-based TAD caller. CNN (two convolutional and one max pooling) extracts features around TAD boundaries from Hi-C maps (10x10 bins), further processed by a Convolutional Block Attention Module, CBAM, and those features are subsequently analyzed by the transformer model. Flattening, two dense layers with dropout, sigmoid, classification into boundary/non-boundary. Wilcoxon test to compare intra- vs. inter-region interactions to further eliminate false positives. Two TADs are merged in a hierarchy if cosine similarity of their interactions is greater than 0.8. KR and VC-normalized matrices, human GM12878, IMR90, K562, NHEK, HUVEC, and HMEC cells, 10kb, 25kb, 50kb, 100kb resolution. Hyperparameters selected based on grid search and precision, recall, and F1-score optimization. Compared with six callers (CaTCH, CHAC, deDoc, DI, TopDom, Arrowhead) on TADs detected by at least two callers. Evaluation metrics: Average peak, fold change, boundary tagged ratio, measure of concordance, TADadjR2. Associations with the RNA polymerase II, H3K4me3, H3K36me3, H3K9me3, CTCF, RAD21, TSS, SINE, and HK genes. TAD number, size comparisons. Python, Tensorflow. <details>
  <summary>Paper</summary>
  Wang, Xiaoyan, Junwei Luo, Lili Wu, Huimin Luo, and Fei Guo. "deepTAD: an approach for identifying topologically associated domains based on convolutional neural network and transformer model." Briefings in Bioinformatics 26, no. 2 (2025): bbaf127. https://doi.org/10.1093/bib/bbaf127
</details>

- <a name="refhic">[RefHiC](https://github.com/BlanchetteLab/RefHiC)</a> - attention-based neural network to predict loops and TADs from local contact submatrices. Uses a reference panel of Hi-C datasets (30 for human, 20 for mouse), provided with the tool. Input - a Hi-C contact map, 5kb resolution. Trained on GM12878 data, using ChIA-PET, HiCHIP on CTCF, RAD21, SMC1, H3K27ac. Trained on [RobusTAD (Python implementation)](https://github.com/zhyanlin/RobusTAD) left and right TAD boundaries, predicts also left and right separately. Some interesting implementations include using focal loss for the loop model training, supervised contrastive learning for the encode pretraining. Outperforms Chromosight, Peakachu, HiCCUPS, Mustache loop callers and CaTCH, Grinch, deDoc, Domailcall, EAST, GMAP, Arrowhead, IC-Finder, HiTAD, TopDom, Armatus, OnTAD TAD callers across different cell types, species, sequencing depths. PyTorch, conda installable, CPU but training requires GPU. <details>
  <summary>Paper</summary>
  Zhang, Y., Blanchette, M. Reference panel guided topological structure annotation of Hi-C data. Nat Commun 13, 7426 (2022). https://doi.org/10.1038/s41467-022-35231-3
</details>
...
- Benchmarking of 13 hierarchical TAD callers, normalization, resolution, sequencing depth, epigenomic features, tool usability (Rao 2014 data [GSE63525](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE63525),5kb-100kb resolution). Grouped into five strategies (Table 1): linear score (Arrowhead, Armatus, CaTCH, HiTAD, matryoshka, OnTAD, Multi-CD), clustering (IC-Finder, TADpole, BHi-Cect, SpectralTAD), network features (HBM, spectral, 3DNetMod, GRiNCH), structural entropy (deDoc, SuperTAD), statistical model (TADtree, GMAP, PSYCHIC, HiKey). Similarity among TADs by average linkage, grouped into four clusters. Generally, TADs are smaller than 2Mb. Resolution affects TAD callers, SpectralTAD the most reproducible. Introduction about TADs subdivided in sub-TADs, enrichment of TAD boundaries in CTCF, cohesin, active epigenomic marks of strong enhancers, mediators, transcription factors, TSSs, TTSs. Evidence that TAD hierarchy is different from stacking Hi-C heatmaps of heterogeneous cells. An air conditioner model making highly interacting TAD boundaries affecting gene expression analogous to the concentration of AC affecting temperature. Differential interactions between GM12878 and K562, chr7, 86-88Mb region, DMTF1, ADAM22, other genes. [Supplementary figures and tables](https://static-content.springer.com/esm/art%3A10.1038%2Fs41467-024-48593-7/MediaObjects/41467_2024_48593_MOESM1_ESM.pdf). [Supplementary Data 2](https://static-content.springer.com/esm/art%3A10.1038%2Fs41467-024-48593-7/MediaObjects/41467_2024_48593_MOESM5_ESM.xlsx) - data references. <details>
  <summary>Paper</summary>
  Xu, J., Xu, X., Huang, D. et al. A comprehensive benchmarking with interpretation and operational guidance for the hierarchy of topologically associating domains. Nat Commun 15, 4376 (2024). https://doi.org/10.1038/s41467-024-48593-7
</details>
...
- DeepChrome - deep convolutional network for binary classification of gene expression (mediad cutoff for high/low) using five histone modification data as input (Roadmap Epigenomics). Visualization of predictive combinatorial histone patterns. H3K36me3, H3K4me1, H3K4me3 predicting high expression, H3K27me3, H3K9me3 - low expression. Outperforms linear regression, SVM, Random Forest. Bins of 100bp +/-5000bp around TSSs, 10 bin step sliding window, 50 filters. <details>
  <summary>Paper</summary>
  Singh, Ritambhara, Jack Lanchantin, Gabriel Robins, and Yanjun Qi. “DeepChrome: Deep-Learning for Predicting Gene Expression from Histone Modifications.” Bioinformatics 32, no. 17 (September 1, 2016): i639–48. https://doi.org/10.1093/bioinformatics/btw427.
</details>
...
- Review of chromatin capture technologies and analytical methods. Details of 3C, 4C, 5C, ChIA-PET, Hi-C and their enhancements (in situ Hi-C, HiChIP, Capture Hi-C, Arima Genomics Hi-C 3.0, others). Table 1 - advantages and disadvantages of Hi-C enhancements. Single-cell variants, Table 2 compares experimental steps. Chromatin structures - chromosomal territories, A/B compartments, TADs, loops, FIREs. Table 5 - TAD calling methods, description of methods for detecting other structures. Highly detailed, excellent learning resource. <details>
  <summary>Paper</summary>
  Cetin, Selinay, and Emre Sefer. “The Significance of Chromosome Conformation Capture in 3D Genome Architecture Comprehension.” Computational Biology and Chemistry 119 (December 2025): 108534. https://doi.org/10.1016/j.compbiolchem.2025.108534.
</details>
...
- **GAGE-seq** - genome architecture and gene expression by sequencing technology. Single-cell 3D genomics and transcriptomics from the same cell. Combinatorial barcoding strategy, 2 four-cut restriction enzymes, CviQI and MseI, both producing the same adhesive DNA end 5′-TA. Greater efficiency than HiRES ( Hi-C and RNA-seq employed simultaneously), resolves more cell types than typical scHi-C Correlations with published datasets (gene expression, A/B compartment scores, insulation scores). Applied to mouse brain cortex (3,296, resolves 28 cell types) and human bone marrow CD34+ cells (2,815, resolve 6 cell types) (also K562 for mixture testing). Integrated with spatial transcriptomics (MERFISH, via scRNA-seq, Seurat method). Gene expression correlates with A/B compartment scores, negative correlation with insulation score. [GitHub](https://github.com/ma-compbio/GAGE-seq), [Jupyter notebook examples](https://github.com/ma-compbio/GAGE-seq/tree/main/scripts_analysis) . [GSE238001](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE238001) - processed data in txt, pairs format. <details>
  <summary>Paper</summary>
  Zhou, Tianming. “GAGE-Seq Concurrently Profiles Multiscale 3D Genome Organization and Gene Expression in Single Cells.” Nature Genetics, https://doi.org/10.1038/s41588-024-01745-3
</details>

- **LiMCA** (Linking mRNA to Chromatin Architecture) - simultaneous single-cell Hi-C and gene expression profiling (not from the same cells). Physically separates the nucleus (whole-genome amplification, META) and the cytoplasm (Smart-seq2 full-length transcriptome). Clustering scA/B values, gene expression, UMAP alignment using Seurat CCA-based algorithm. Applied to the mouse olfactory system development, K562, eHAP, BJ human cell lines. Combined with scATAC-seq (METATAC, processed with ArchR). [Smart-seq2 processing](https://broadinstitute.github.io/warp/docs/Pipelines/Smart-seq2_Single_Sample_Pipeline/README/), [hikit](https://github.com/lh3/hickit) and [dip-c](https://github.com/tanlongzhi/dip-c) for single-cell Hi-C processing. Pairtools, cooltools, juicer, chromosight. Raw data at [PRJNA1002315](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA1002315), processed data (hic, pairs, txt) at [GSE240128](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE240128). [GitHub](https://github.com/zhang-jiankun/LiMCA). <details>
  <summary>Paper</summary>
  Wu, Honggui, Jiankun Zhang, Fanchong Jian, Jinxin Phaedo Chen, Yinghui Zheng, Longzhi Tan, and X. Sunney Xie. “Simultaneous Single-Cell Three-Dimensional Genome and Gene Expression Profiling Uncovers Dynamic Enhancer Connectivity Underlying Olfactory Receptor Choice.” Nature Methods, April 15, 2024. https://doi.org/10.1038/s41592-024-02239-0.
</details>

- **GAGE-seq** - single-cell assay for simultaneous measurement of the 3D genome structure and gene expression (genome architecture and gene expression by sequencing). Combinatorial barcoding of cross-linked RNA and chromatin, two 4-cut restriction enzymes, fragmentation, barcoding, separation, separate sequencing of matched scHi-C and scRNA-seq profiles. Benchmarked on a mixture of human K562 and mouse NIH3T3 cells. Compared to HiREA method, higher throughput, more cost effective. Applied to mouse cortex, 28 cell types, 3D is associated with gene expression and spatial locations. Applied to developmental stages of human hematopoiesis timecourse, 3D and expression association is not straightforward. <details>
  <summary>Paper</summary>
  Zhou, Tianming, Ruochi Zhang, Deyong Jia, Raymond T. Doty, Adam D. Munday, Daniel Gao, Li Xin, Janis L. Abkowitz, Zhijun Duan, and Jian Ma. “Concurrent Profiling of Multiscale 3D Genome Organization and Gene Expression in Single Mammalian Cells.” Preprint. Genomics, July 25, 2023. https://doi.org/10.1101/2023.07.20.549578.
</details>
...
- Micro-C (MNase digestion Hi-C) data analysis. Nucleosome-level (~100-200bp) resolution Hi-C, captures all structures in regular Hi-C data. Stripes/flames structures correspond to enhancer-promoter interactions, colocalize with PolII, CTCF. E-P stripes and domains interact with each other to form nested structures at scales of tens to hundreds of kilobases. Integrating Micro-C contact maps with 48 public genome datasets, PCA and tSNE on them to separate boundaries into different categories. Fine-scale structures exist in compartment A but not B, associated with gene structures, encompass two genes and have a flanking 5-40kb region, colocalize with CpG islands, promoters, TFBSs, fragile nucleosomes, and tRNA genes (Fig. 2e). Boundaries in the inactive compartments are colocalized with repeats. Features associated with location (Pos: CTCF & Cohesin, ATAC, H3K4me3; Neg: H3K27me3, H3K9me2, Cbx) and boundary strength (Pos: ATAC, H3K4me3, PolII, CTCF & Cohesin) (Fig. 3B,C). Enhancer-related boundaries separate in two groups, (I) cell type-specific, enriched with H3K4me1, Essrb, Nanog, and (II) constitutive, enriched with H3K27ac, Mediator, and Yy1. CTCF or cohesin, chromatin accessibility, Mediator, active histone marks, and transcription factors are all positive predictors of boundary location, while H3K27me3, H3K9me2, and CBX proteins are negative predictors of boundary location. CTCF & Cohesin are predictive of distant interactions. PolII, TFs and coactivators, and active histone modifications are predictive of short-range interactions. TADs are split into "micro TADs", subdivided into five subgroups. Two-start zig-zag model tetra-nucleosome stacks. Mouse embryonic stem cell, 38 biological replicates, merged. [Supplementary](https://www.cell.com/molecular-cell/fulltext/S1097-2765(20)30150-7#supplementaryMaterial): Table S3 - public datasets. Raw and processed (Cooler/Juicebox/Higlass) data at [GSE130275](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE130275). 4DN: 4DNES14CNC1I, 4DNESSY8C22T, 4DNES7X5GQUR - boundaries and insulation score. [Twitter/X](https://twitter.com/Stanley_TH/status/1242843692081111040?s=20). <details>
    <summary>Paper</summary>
...
- **Multiplex-GAM** technology, a faster and more affordable version of  genome architecture mapping (GAM). Combining multiple independent nuclear profiles. The SLICE statistical model of chromatin interactions in GAM data, extended for multiplex-GAM. Compared with the canonical GAM data of 408 mouse embryonic stem cells, bulk and single-cell Hi-C. Performs similar to GAM, detects similar TAD boundaries. GAM detects strong contacts missing in Hi-C, these contacts are 'active' regions. Detects multi-way (triplet) interactions missing in Hi-C. [GSE166381](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE166381) - processed data. [GitHub](https://github.com/pombo-lab/multiplex-gam-2023/tree/main) - analysis scripts, Jupyter notebooks. <details>
  <summary>Paper</summary>
  Beagrie, Robert A., Christoph J. Thieme, Carlo Annunziatella, Catherine Baugher, Yingnan Zhang, Markus Schueler, Alexander Kukalev, et al. “Multiplex-GAM: Genome-Wide Identification of Chromatin Contacts Yields Insights Overlooked by Hi-C.” Nature Methods, June 19, 2023. https://doi.org/10.1038/s41592-023-01903-1.
</details>
...
- **Tri-C technology**, a 3C approach to characterize multi-way chromatin interactions at individual alleles (viewpoints). Compared with limitations of Hi-C, GAM, SPRITE. NlaIII 4-cutter restriction enzyme producing approx. 200bp fragments.
Demonstrated on the structure of the mouse globin locus (erythroid cells with active globin genes vs. embryonic stem cells with the silent globin loci). High-resolution single-allele chromatin structures do not support a model of stable loop formation between individual CTCF-binding sites but indicate a dynamic mechanism, such as loop extrusion underlying the for- mation of chromatin domains. Github repos for [CCseqBasicF](https://github.com/Hughes-Genome-Group/CCseqBasicF) Capture-C and [TriC](https://github.com/oudelaar/TriC/) Tri-C analysis. Data on GEO [GSE107940](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE107940). <details>
  <summary>Paper</summary>
  Oudelaar, A. Marieke, James O. J. Davies, Lars L. P. Hanssen, Jelena M. Telenius, Ron Schwessinger, Yu Liu, Jill M. Brown, et al. “Single-Allele Chromatin Interactions Identify Regulatory Hubs in Dynamic Compartmentalized Domains.” Nature Genetics 50, no. 12 (December 2018): 1744–51. https://doi.org/10.1038/s41588-018-0253-2.
</details>
